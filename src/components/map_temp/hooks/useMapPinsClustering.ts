...
export const useMapPinsClustering = ({
  ...
  // 🛡️ Dedicated, isolated Layer Groups to guarantee 100% rock-solid stability and zero DOM thrashing
  const districtsGroupRef = useRef<any>(null);
  const gatesGroupRef = useRef<any>(null);
  const businessesGroupRef = useRef<any>(null);
  const targetBuildingGroupRef = useRef<any>(null);
  const pickerGroupRef = useRef<any>(null);
  const districtPolygonsRef = useRef<Array<{ letterAr: string; polygon: any; color: string }>>([]);
  const isInitialMountRef = useRef<boolean>(true);
  const prevSelectedZoneRef = useRef<string | undefined>(undefined);

  // 1. Initialize layer groups once on map mount
  useEffect(() => {
    const map = leafletMapRef.current;
    if (!map || !window.L) return;

    if (!districtsGroupRef.current) districtsGroupRef.current = window.L.layerGroup().addTo(map);
    if (!gatesGroupRef.current) gatesGroupRef.current = window.L.layerGroup().addTo(map);
    if (!businessesGroupRef.current) businessesGroupRef.current = window.L.layerGroup().addTo(map);
    if (!targetBuildingGroupRef.current) targetBuildingGroupRef.current = window.L.layerGroup().addTo(map);
    if (!pickerGroupRef.current) pickerGroupRef.current = window.L.layerGroup().addTo(map);

    return () => {
      districtsGroupRef.current?.remove();
      districtsGroupRef.current = null;
      gatesGroupRef.current?.remove();
      gatesGroupRef.current = null;
      businessesGroupRef.current?.remove();
      businessesGroupRef.current = null;
      targetBuildingGroupRef.current?.remove();
      targetBuildingGroupRef.current = null;
      pickerGroupRef.current?.remove();
      pickerGroupRef.current = null;
      districtPolygonsRef.current = [];
    };
  }, [leafletMapRef.current]);

  // 2. 🗺️ Hadayek Districts: Rendered ONCE. Never cleared on pan or zoom, perfectly static on Hadayek Al-Ahram
  useEffect(() => {
    const map = leafletMapRef.current;
    ...
    if (!showDistrictsOverlay) return;
      ...
      district.polygons.forEach((polyCoords) => {
        ...
        polygon.on('click', (e: any) => {
          ...
          if (map && district.polygons && district.polygons[0]) {
            ...
            map.flyToBounds(bounds, { padding: [40, 40], maxZoom: 17, duration: 0.9 });
          }
        });
  ...
  }, [leafletMapRef.current, showDistrictsOverlay, onSelectZone]);

  // 3. ⚡ Instantaneous style update when selectedZone changes & user-initiated smooth camera pan
  useEffect(() => {
    if (!districtPolygonsRef.current || !districtPolygonsRef.current.length) return;

    const prevZone = prevSelectedZoneRef.current;
    prevSelectedZoneRef.current = selectedZone;

    // Update only the previous and current polygons to completely eliminate SVG DOM thrashing & jitter
    districtPolygonsRef.current.forEach(({ letterAr, polygon, color }) => {
      if (letterAr === selectedZone) {
        polygon.setStyle({
          color: color,
          weight: 3.5,
          opacity: 1.0,
      ...
      } else if (letterAr === prevZone || !prevZone) {
        polygon.setStyle({
          color: color,
          weight: 1.5,
          opacity: 0.85,
    ...
    // 🛑 Lock map firmly on Hadayek Al-Ahram bounds on initial mount.
    // The camera ONLY flies if the user explicitly triggers a zone change after mount!
    if (isInitialMountRef.current) {
      isInitialMountRef.current = false;
      return;
    }

    if (selectedZone && leafletMapRef.current && window.L) {
      const cleanZone = selectedZone.replace(/^منطقة\s+/, '').trim();
      const matched = HADAYEK_OFFICIAL_DISTRICTS.find(
        (d) => d.letterAr === selectedZone || d.letterAr === cleanZone || d.nameAr === selectedZone
      );
      if (matched && matched.polygons && matched.polygons[0]) {
        try {
          const bounds = window.L.latLngBounds(matched.polygons[0]);
          leafletMapRef.current.flyToBounds(bounds, { padding: [35, 35], maxZoom: 16, duration: 0.8 });
        } catch {}
      }
    }
  }, [selectedZone]);

  // 3.5 🎯 Direct Business Focus: Smoothly center camera when a business is selected via link or click
  useEffect(() => {
    if (!selectedBusiness || !leafletMapRef.current) return;
    const { lat, lng } = selectedBusiness;
    if (typeof lat === 'number' && typeof lng === 'number' && lat > 0 && lng > 0) {
      leafletMapRef.current.flyTo([lat, lng], 17, { duration: 1.0 });
    }
  }, [selectedBusiness?.id, selectedBusiness?.lat, selectedBusiness?.lng]);

  // 4. 🚪 Hadayek Official Gates: Rendered in dedicated group
  useEffect(() => {
    const map = leafletMapRef.current;
    const group = gatesGroupRef.current;
    if (!map || !group || !window.L) return;
    ...
    HADAYEK_OFFICIAL_GATES.forEach((gate) => {
      ...
      `);
      group.addLayer(gateMarker);
    });
  }, [leafletMapRef.current, showHadayekGates, showGatesLayer]);

  // 5. 📍 Picker Mode Needle & Accuracy Circle
  useEffect(() => {
    const map = leafletMapRef.current;
  ...
  }, [leafletMapRef.current, mode, currentLat, currentLng, gpsAccuracy, updateSelectedPosition]);

  // 6. 📍 Target Building Pin & Radius Circle
  useEffect(() => {
    const map = leafletMapRef.current;
    const group = targetBuildingGroupRef.current;
    if (!map || !group || !window.L) return;
    ...
    if (
      ...
      !targetBuilding ||
      !showTargetPin ||
      typeof targetBuilding.lat !== 'number' ||
      typeof targetBuilding.lng !== 'number'
    ) {
      return;
    }
    ...
    group.addLayer(bldgCircle);

    map.flyTo([targetBuilding.lat, targetBuilding.lng], 17, { duration: 1.0 });
  }, [leafletMapRef.current, mode, targetBuilding, showTargetPin]);

  // 7. 🎯 Businesses Layer (Strictly Item 7: Completely hidden unless an explicit category filter is chosen!)
  useEffect(() => {
    const map = leafletMapRef.current;
    const group = businessesGroupRef.current;
    if (!map || !group || !window.L) return;
    ...
    // 🛑 Item 7: Total stop of mass business display! Pins only appear when a specific category is active
    const hasCategoryFilter = Boolean(mapCategoryFilter && mapCategoryFilter !== 'all');
    if (!hasCategoryFilter) {
      return; // Clean cadastral map with zero business clutter!
    }
    ...
    clusters.forEach((cluster) => {
      if (cluster.items.length === 1) {
        ...
        const showFullPill = zoomLevel >= 17;
